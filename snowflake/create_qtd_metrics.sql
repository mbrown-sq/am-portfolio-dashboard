-- QTD Metrics Snapshot
-- Creates a table with quarter-to-date metrics per AM

CREATE OR REPLACE TABLE app_merch_growth.mbrown_sandbox.au_smb_qtd_metrics AS
...
SELECT
  ...
  COUNT(DISTINCT CASE WHEN act.IS_DM = 1 THEN act.BUSINESS_ID END) AS dm_conversations,
  COUNT(DISTINCT act.BUSINESS_ID) AS businesses_touched,
...
GROUP BY LOWER(ame.LDAP), ame.FIRST_NAME;

-- Verify
SELECT * FROM app_merch_growth.mbrown_sandbox.au_smb_qtd_metrics
